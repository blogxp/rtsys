        ...
        table: {
            ...
            init: function(options) {
                var defaults = {
                    ...
                    url: window.b5go.bootUrl.url,
                    createUrl: window.b5go.bootUrl.createUrl,
                    updateUrl: window.b5go.bootUrl.updateUrl,
                    removeUrl: window.b5go.bootUrl.removeUrl,
                    removeAllUrl: window.b5go.bootUrl.removeAllUrl,
                    clearCacheUrl: window.b5go.bootUrl.clearCacheUrl,
                    method: 'post',
                    height: undefined,
                ...
                $.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function() {
                    ...
                    $.post(table.options.exportUrl, dataParam, function(result) {
                        if (result.code == web_status.SUCCESS) {
                            window.open(urlcreate(window.b5go.bootUrl.downUrl,"fileName=" + encodeURI(result.msg) + "&delete=true"));
        ...
        operate: {
            ...
            statusChange:function(obj){
                ...
                $.modal.confirm("确认要" + name + "该" + title + "吗？", function() {
                    $.operate.post(window.b5go.bootUrl.statusUrl, { "id": id, "status": status, name:name, field:field });
                });
            },
            // 详细信息
