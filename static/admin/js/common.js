                    ...
                    onImageUpload: function (files) {
                        for (var i=0;i<files.length;i++){
                            sendFile(files[i],{cat:'editor'},function (data) {
                                thisObj.summernote('editor.insertImage', data.url, data.origin_name);
                            });
                        }
                    }
                }
...
// 将ajax请求改为application/json 并将data进行json序列化
function ajaxParseData(xhr,params){
    if (params.type.toLowerCase() !== "post") return;
    if (params.url.indexOf("/common/upload_") >=0) return;
    if (params.contentType === false) return;

    xhr.setRequestHeader("Content-type", "application/json");// 设置Content-type
    params.data = JSON.stringify(Qs.parse(params.data))
}
/** 设置全局ajax处理 */
$.ajaxSetup({
