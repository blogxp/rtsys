...
function b5uploadfilehtml(path,name,url,filename){
    ...
    inputname = inputname=="1"?true:false;
    url = url?url:path;
...
function b5uploadfileinit(id,callback) {
    ...
    layui.use('upload', function(){
        layui.upload.render({
            elem: '#'+id //绑定元素
            ,url: window.b5go.upFileUrl //上传接口
            ,field:'file'
            ,multiple:multi
            ,number:maxnum
            ,data:{
            ...
            ,done: function(res){
                if(res.success && res.code===0){
                    if($.common.isFunction(callback)){
                        ...
                        var html=b5uploadfilehtml(res.data.path,id,res.data.url,res.data.origin_name);
                        b5uploadhtmlshow(id,html);
                    }
                }else{
                    $.modal.msgError(res.msg);
